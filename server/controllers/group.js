    ...
    .then((group) => {
      ...
      if (
        group.members.filter(
          (member) => member.userId._id.toString() === decodedToken.id,
        ).length === 0
      ) {
      ...
      const dailies = group.tasks.dailies
        .filter((task) =>
          task.participants.find(
            (part) => part.userId.toString() === decodedToken.id,
          ),
        )
        .map((task) => ({
          difficulty: task.difficulty,
          done: task.participants.find(
            (part) => part.userId.toString() === decodedToken.id,
          ).done,
          _id: task.taskId._id,
      ...
      const todos = group.tasks.todos.filter((task) =>
        task.participants.find(
          (part) => part.userId.toString() === decodedToken.id && !part.done,
        ),
      );
      ...
      return res
        .status(200)
        .json({
          isLeader: group.leader.toString() === decodedToken.id,
          group: responseData,
        });
    })
    .catch((err) => {
...
exports.getMembers = (req, res, next) => {
  ...
  Group.findById(groupId)
    ...
    .then((group) => {
      ...
      if (omitLeader) {
        formedMembers = formedMembers.filter(
          (member) => member.id.toString() !== group.leader.toString(),
        );
      }
...
exports.postCreate = (req, res, next) => {
  ...
  Group.findOne({ name })
    .then((groupDoc) => {
      if (groupDoc) {
        // checks if the user already has an own group
        return res
          .status(500)
          .json({ message: "Group already exists with given name!" });
      }
...
exports.postAddTask = (req, res, next) => {
  ...
  const task = new Task({
    // create new task and fill it up with data
    ownerId: groupId,
    ownerType: "Group",
    name,
    date,
  ...
  Task.findById(taskId)
    .then((task) => {
      Group.findById(task.ownerId)
        .then((group) => {
          const todoToCheck = group.tasks.todos.find(
            (task) => task.taskId.toString() === taskId,
          );
          const participantTocheck = todoToCheck.participants.find(
            (part) => part.userId.toString() === decodedToken.id,
          );
...
exports.postCheckDaily = (req, res, next) => {
  ...
  Task.findById(taskId)
    .then((task) => {
      Group.findById(task.ownerId)
        .then((group) => {
          const dailyToCheck = group.tasks.dailies.find(
            (task) => task.taskId.toString() === taskId,
          );
          const participantTocheck = dailyToCheck.participants.find(
            (part) => part.userId.toString() === decodedToken.id,
          );
          participantTocheck.done = true;
    ...
    .then((task) => {
      ...
      return Group.findById(task.ownerId)
        .then((group) => {
          ...
          if (isDaily) {
            group.tasks.dailies = group.tasks.dailies.filter(
              (task) => task.taskId.toString() !== taskId,
            );
          } else {
            console.log(group.tasks.todos);
            group.tasks.todos = group.tasks.todos.filter(
              (task) => task.taskId.toString() !== taskId,
            );
            console.log(group.tasks.todos);
          }
