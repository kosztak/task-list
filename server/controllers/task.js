...
exports.patchGroupTask = (req, res, next) => {
    ...
    Task.findById(id)
        .then(task => {
            task.updateOne({ $set: collectedTaskData }) // update task in database
                .then(() => {
                ...
                .then(group => {
                    ...
                    const participantList = groupData.participants.map(part => {
                        const partIndex = groupTask.participants.findIndex(groupPart => groupPart.userId.toString() === part);

                        return {
                            userId: part,
                            ...(partIndex !== -1? { done: groupTask.participants.at(partIndex).done } : { done: false })
                        }
                    });
