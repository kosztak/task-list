...
export default function EditGroupTaskPage() {
  ...
  function handleTaskDelete() {
    axiosInstance
      .delete(`/group/task?taskId=${task.id}&isDaily=${isDaily}`)
      .then(() => {
        ...
        <>
          ...
          <Form method="POST" className="flex flex-col items-stretch gap-16">
            <div
              ...
              style={{
                gridTemplateAreas:
                  '"name description" "period description" "date gap" "difficulty difficulty" "participants participants"',
              }}
            >
              <Input
                type="text"
              ...
              {isDaily && (
                <Input
                  ...
                  name="gap"
                  min="1"
                  defaultValue={task.gap}
                  style={{ gridArea: "gap" }}
                />
              )}
              <div style={{ gridArea: "difficulty" }}>
                <p className="text-lg font-bold text-dark text-center mb-4">
                  Difficulty
                </p>
                <div className="flex justify-evenly border-2 border-dark p-2 rounded">
                  <RadioButton
                    ...
                    name="difficulty"
                    value="trivial"
                    defaultChecked={task.difficulty === "trivial"}
                  />
                  <RadioButton
                    text="Easy"
                    type="radio"
                    name="difficulty"
                    value="easy"
                    defaultChecked={task.difficulty === "easy"}
                  />
                  <RadioButton
                    text="Medium"
                    type="radio"
                    name="difficulty"
                    value="medium"
                    defaultChecked={task.difficulty === "medium"}
                  />
                  <RadioButton
                    text="Hard"
                    type="radio"
                    name="difficulty"
                    value="hard"
                    defaultChecked={task.difficulty === "hard"}
                  />
                </div>
              </div>
                ...
                <div>
                  {members.map((member, index) => (
                    <Checkbox
                      ...
                      name={"participant"}
                      value={member.id}
                      defaultChecked={task.participants.some(
                        (part) => part === member.username,
                      )}
                    />
                  ))}
                </div>
...
export async function loader({ request, params }) {
  // loads the members of group from back-end
  const members = await axiosInstance
    .get(`/group/members?groupId=${params.groupId}`)
    .then((response) => {
  ...
  const task = await axiosInstance
    .get(
      `/group/task-data?groupId=${params.groupId}&type=${params.type}&taskId=${params.taskId}`,
    )
    .then((response) => {
...
export async function action({ request, params }) {
  // updates the current task on the back-end
  ...
  if (globalTask.period) {
    // overrides the renewel property of task, if it is a daily
    taskData = {
      ...taskData,
      ...(globalTask.period !== data.get("period") && {
        "renewel.period": data.get("period"),
      }),
      ...(globalTask.gap !== parseInt(data.get("gap")) && {
  ...
  const groupData = {
    ...(globalTask.difficulty !== data.get("difficulty") && {
  ...
  return axiosInstance
    .patch("/task/group-task", {
      taskData,
      groupData,
      isDaily: data.get("period") !== undefined,
    })
    .then(() => {
