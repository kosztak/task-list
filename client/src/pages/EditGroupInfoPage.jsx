...
export default function EditGroupInfoPage() {
  ...
  function handleKickOutClick(userId) {
    axiosInstance
      .delete(`/group/member?groupId=${params.groupId}&userId=${userId}`)
      .then(() => {
        navigate("");
      })
      .catch((err) => {
    ...
    <div className="bg-white rounded-lg p-4 flex flex-col items-stretch gap-12">
      ...
      <div className="bg-dark rounded-lg p-4 grid grid-cols-[1fr_1fr] gap-12">
        {members.length > 0 ? (
          ...
          <p className="text-white text-2xl font-bold text-center col-span-2">
            There's no members in this group!
          </p>
        )}
      </div>
    </div>
...
export async function loader({ request, params }) {
  ...
  return axiosInstance
    .get(`/group/members?groupId=${params.groupId}&omitLeader=${true}`)
    .then((response) => {
...
export async function action({ request, params }) {
  ...
  if (data.get("password") === data.get("password-again")) {
    // cheks if given passwords are matching
    const requestData = {
