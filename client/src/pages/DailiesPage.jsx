...
export default function DailiesPage() {
  ...
  function generateDailyList() {
    // creates a DailyBar component list to show all user related dailies
    return dailiesList.map((daily) => {
      return <DailyBar key={daily._id} task={daily} isUser={true} />;
    });
  }
  ...
  return (
    <div className="bg-white rounded-lg p-4 flex flex-col items-stretch gap-8">
      ...
      {!dailiesList || dailiesList.length === 0 ? (
        ...
        <div className="flex flex-col gap-4">{generateDailyList()}</div>
      )}
    </div>
  );
}
